 ...
 *  Copyright 2013 Huzefa Fatakdawala
 */
...
//		map = new MapView(getApplicationContext(), 500, 300, 23, 25);
		...
		class RotationVectorEventListener implements SensorEventListener {
			...
			public void onSensorChanged(SensorEvent event) {
				...
				if(wallComingUp.size() != 0) {
					wallText.setText("Wall detected!");
				}else {
					wallText.setText("No wall. If a path is not detected or is not satisfactory" +
						", turn around in the same spot.");
				}
				if(wallComingUp.size() != 0) {
					isWall = true; //This condition disables the step counter and accelerometer updates
				}else {
					isWall = false;
				}
				...
				if(noBlocksIntercept.size() == 0) {
					 ...
					 * This was, in my opinion a fault in the SVG maps provided. I decided to
					 * exploit this.
					 */
